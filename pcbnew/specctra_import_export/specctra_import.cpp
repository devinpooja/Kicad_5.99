...
bool PCB_EDIT_FRAME::ImportSpecctraSession( const wxString& fullFileName )
{
    ...
    try
    {
      ImportBoardToSpecctraSession( GetBoard(), fullFileName );
    }
    ...
    if( ok )
    {
        SetStatusText( wxString( _( "BOARD imported OK." ) ) );
    }
    else
    {
        DisplayErrorMessage( this, _( "Unable to import, please fix and try again" ), errorText );
    }

    return ok;
}

void ImportBoardToSpecctraSession( BOARD* aBoard, const wxString& fullFileName )
{
    SPECCTRA_DB     db;
    LOCALE_IO       toggle;
    ...
    extern void OnModify(void);
    //OnModify();

    aBoard->GetConnectivity()->Clear();
